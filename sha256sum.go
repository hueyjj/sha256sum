...
func Sha256Sum(msg string) (string, error) {
	...
	// Preprocessing
	bmsg := []byte(msg)
	length := uint64(len(bmsg))
	bmsg = append(bmsg, 0x80)
	K := 448 - len(bmsg)*8 // k, the numer of zeroes for padding to 512-bit
	padding := make([]byte, K/8)
	bmsg = append(bmsg, padding...)
	blength := make([]byte, 8)
